...
// ─── Tijdsvergelijking (met tijdzone-ondersteuning) ──────────────────────────

function tijdMatchNu(tijdStr, tz) {
  if (!tijdStr) return false;
  const userTz = tz || 'Europe/Brussels';
  const now = new Date();
  // Zet server-UTC om naar de tijdzone van de gebruiker
  const localStr = now.toLocaleTimeString('en-GB', {
    timeZone: userTz,
    hour: '2-digit',
    minute: '2-digit',
    hour12: false
  ...
  const [remH, remM] = tijdStr.split(':').map(Number);
  const nowMin = localH * 60 + localM;
  const remMin = remH * 60 + remM;
      ...
      try {
        ...
        if (reminders.glucoseEnabled && Array.isArray(reminders.glucoseTimes)) {
          for (const r of reminders.glucoseTimes) {
            if (r.enabled && tijdMatchNu(r.time, userTz)) {
              messages.push({
        ...
        if (tijdMatchNu('08:00', userTz) && sub.sensor_data) {
